...
void *connection_handler(void *socket_desc)
{
    ...
    {
    	send = 0;
    	if(msg.select == PASSANGER_STATUS_BY_ID_COMMAND)
    	{
    		memset(msg.message,0, sizeof msg.message);
    	    msg.id = getPassangerStatusById(&Sofia, &msg);

    	    switch(msg.id)
    	    {
    	        case LANDED:
    	        	strcpy(msg.message,strcat(msg.message, "The passanger has landed successfully\n"));
            	    //strcpy(msg.message,"The passanger has landed successfully\n");
					break;
    	        case STILL_FLYING:
    	        	strcpy(msg.message,strcat(msg.message, "The passanger is still flying\n"));

					//strcpy(msg.message,"The passanger is still flying\n");
					break;
    	        case CRASHED:
    	        	strcpy(msg.message,strcat(msg.message, "Sorry but the plane has crashed ;(\n The passenger is dead\n"));

					//strcpy(msg.message,"Sorry but the plane has crashed ;(\n The passenger is dead\n");
					break;
    	        case NOT_ONBOARD:
    	        	strcpy(msg.message,strcat(msg.message, "Sorry, the passanger isn't using our services at the moment\n"));
			...
			if(send <= 0)
				puts("Unable to send passanger information to client");
        }
    	else if(msg.select == FLIGHT_STATUS_BY_ID_COMMAND)
    	{
    		msg.id = getFlightStatusById(&Sofia,msg.id);

    		    	    switch(msg.id)
    		    	    {
    		    	        case LANDED:
    		            	   strcpy(msg.message,"The flight has landed successfully\n");
    							break;
    		    	        case STILL_FLYING:
    							strcpy(msg.message,"Flight in progress...\n");
    							break;
    		    	        case CRASHED:
    							strcpy(msg.message,"Sorry but the plane has crashed ;(\n");
    							break;
    		    	        case FLIGHT_NOT_FOUND:
    							strcpy(msg.message,"There is no current flight with the given id\n");
    							break;
    						default:
    							break;
    		    	    }
    					send = write(sock, &msg, sizeof(Message));
    					if(send <= 0)
    						puts("Unable to send flight information to client");
    	}
...
//API functions
int getPassangerStatusById(Airport* airport,Message* msg)
{
    int i,j;
    for(i = 0; i < airport->used; i++)
    {
		...
		{
			if(airport->flights_array[i].passangers_array[j].ID == msg->id)
			{
				strcpy(msg->message,"Passanger info: \n");
				strcpy(msg->message,strcat(msg->message, airport->flights_array[i].passangers_array[j].first_name));
				strcpy(msg->message,strcat(msg->message, "\n"));
	        	strcpy(msg->message,strcat(msg->message, airport->flights_array[i].passangers_array[j].second_name));
				strcpy(msg->message,strcat(msg->message, "\n"));
	        	strcpy(msg->message,strcat(msg->message, airport->flights_array[i].passangers_array[j].last_name));
				strcpy(msg->message,strcat(msg->message, "\n"));
...
int getFlightStatusById(Airport* airport,int flightId)
{
    int i;
    for(i = 0; i < airport->used; i++)
    {
			if(airport->flights_array[i].ID == flightId)
			{
				return(airport->flights_array[i].has_landed_flag);
			}
    }
